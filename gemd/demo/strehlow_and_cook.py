...
def formula_latex(old):
    ...
    return re.sub(
        r"(?<=[A-Za-z])([\d\.]+)(?=[A-Za-z]|$)", r"$_{\1}$", formula_clean(old)
    )
...
def make_templates(template_scope=DEMO_TEMPLATE_SCOPE):
    ...
    attribute_feed = {
        ...
        "Crystallinity": [
            ConditionTemplate,
            CategoricalBounds(["Amorphous", "Polycrystalline", "Single crystalline"]),
        ],
        "Color": [
            PropertyTemplate,
            CategoricalBounds(
        ...
        "Temperature derivative of band gap": [
            PropertyTemplate,
            RealBounds(lower_bound=-0.01, upper_bound=0.01, default_units="eV/K"),
        ],
        "Lasing": [PropertyTemplate, CategoricalBounds(["True", "False"])],
        "Cathodoluminescence": [PropertyTemplate, CategoricalBounds(["True", "False"])],
        "Mechanical luminescence": [
            PropertyTemplate,
            CategoricalBounds(["True", "False"]),
        ],
        "Photoluminescence": [PropertyTemplate, CategoricalBounds(["True", "False"])],
        "Electroluminescence": [PropertyTemplate, CategoricalBounds(["True", "False"])],
        "Thermoluminescence": [PropertyTemplate, CategoricalBounds(["True", "False"])],
        "Morphology": [ConditionTemplate, CategoricalBounds(["Thin film", "Bulk"])],
    ...
    object_feed = {
        ...
        "Chemical": [MaterialTemplate, {"properties": [tmpl["Formula"]]}],
...
def make_strehlow_objects(table=None, template_scope=DEMO_TEMPLATE_SCOPE):
    ...
    # Specs
    msr_spec = MeasurementSpec(name="Band gap", template=tmpl["Band gap measurement"])
    ...
    def real_mapper(prop):
        ...
        if "uncertainty" in prop["scalars"][0]:
            if prop["units"] == "eV":  # Arbitrarily convert to attojoules
                mean = convert_units(
                    value=float(prop["scalars"][0]["value"]),
                ...
                val = NormalReal(mean=mean, units="aJ", std=std)
            else:
                val = NormalReal(
                    mean=float(prop["scalars"][0]["value"]),
                    units=prop["units"],
                    std=float(prop["scalars"][0]["uncertainty"]),
                )
        else:
            val = NominalReal(
                nominal=float(prop["scalars"][0]["value"]), units=prop["units"]
            )
        return val

    content_map = {
        RealBounds: real_mapper,
        CategoricalBounds: lambda prop: NominalCategorical(
            category=prop["scalars"][0]["value"]
        ),
    ...
    for row in table:
        ...
        if formula not in compounds:
            compounds[formula] = MaterialSpec(
                ...
                process=ProcessSpec(
                    name="Sample preparation", template=tmpl["Sample preparation"]
                ),
            )
        ...
        origin_map = {"EXPERIMENTAL": Origin.MEASURED, "COMPUTATIONAL": Origin.COMPUTED}
        seen = (
            set()
...
def make_strehlow_table(compounds):
    ...
    output["headers"].append(
        {
            "name": [chem_mat_tmpl.name, chem_tmpl.name],
            "primitive": False,
...
def make_display_table(structured):
    ...
    table[0].insert(
        ...
        "~".join(
            column["name"]
            + ["Std Deviation({})".format(column["bounds"].default_units)]
        ),
    )
    ...
    for row in structured["content"]:
        ...
        # And insert band gap
        table[-1].insert(
            i_bandgap + 1, uncert_map[type(row[i_bandgap])](row[i_bandgap])
        )
