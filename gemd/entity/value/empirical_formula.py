    ...
    def formula(self, value: str):
        if value is None:
            ...
            if not EmpiricalFormula._elements(value).issubset(_all_elements):
                unknown = sorted(
                    EmpiricalFormula._elements(value).difference(_all_elements)
                )
                raise ValueError(
                    "Formula {} contains unknown elements: {}".format(
                        value, " ".join(unknown)
                    )
                )
            self._formula = value
            ...
            raise TypeError(
                "Formula must be given as a string; got {}".format(type(value))
            )
