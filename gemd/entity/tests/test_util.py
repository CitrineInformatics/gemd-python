...
from gemd.entity.util import make_instance, complete_material_history
...
def test_serialized_history():
    ...
    buy_cookie_dough = ProcessRun(
        "Buy cookie dough", uids={"id": "32283"}, spec=buy_spec
    )
    cookie_dough = MaterialRun(
        "cookie dough", process=buy_cookie_dough, spec=cookie_dough_spec
    )
    bake = ProcessRun(
        "bake cookie dough",
        conditions=[
    ...
    IngredientRun(
        material=cookie_dough, process=bake, number_fraction=NominalReal(1, "")
    )
    cookie = MaterialRun("cookie", process=bake, tags=["chocolate chip", "drop"])
    MeasurementRun(
        "taste",
        material=cookie,
        properties=[Property("taste", value=DiscreteCategorical("scrumptious"))],
    )
    ...
    for entity in cookie_history:
        assert (
            len(entity["uids"]) > 0
        ), "Serializing material history should assign uids."
    ...
    cookie_dough_spec_dict = next(
        x for x in cookie_history if x.get("type") == "material_spec"
    )
    buy_cookie_dough_dict = next(
        x for x in cookie_history if x.get("name") == "Buy cookie dough"
    )
