...
def test_flatten_empty_history():
    ...
    ingredient_run = IngredientRun(
        material=input_run, process=transform_run, spec=ingredient
    )
...
def test_repeated_objects():
    ...
    ct = ConditionTemplate(
        name="color", bounds=CategoricalBounds(categories=["black", "white"])
    )
    ...
    ps = ProcessSpec(
        ...
        conditions=Condition(
            name="Paint color", value=NominalCategorical("black"), template=ct
        ),
    )
